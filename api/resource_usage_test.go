...
var _ = Describe("API", func() {
	...
	var send = func(method, ct, path string, data io.Reader) interface{} {
		var v interface{}
	...
	var post = func(path string, data io.Reader) interface{} {
		return send("POST", echo.MIMEApplicationForm, path, data)
	}

	var del = func(path string, data io.Reader) interface{} {
		return send("DELETE", echo.MIMEApplicationForm, path, data)
	}

	var put = func(path string, data io.Reader) interface{} {
	...
	Context("POST /pricing_plans", func() {
		...
		It("should create a pricing plan (form POST)", func() {
			...
			out := post(path, strings.NewReader(form.Encode()))
			ExpectJSON(out, map[string]interface{}{
				"id":         1,
				"name":       "NewPlan",
	...
	Context("/pricing_plans/:id", func() {
		...
		It("should update a pricing plan (via form PUT)", func() {
			...
			out := put(path, strings.NewReader(form.Encode()))
			ExpectJSON(out, map[string]interface{}{
				"formula":    "10*10",
				"id":         11,
				"name":       "UpdatedPlan",
	...
	Context("/pricing_plans/:id", func() {
		...
		It("should delete a pricing plan (via form DELETE)", func() {
			form := url.Values{}
			out := del(path, strings.NewReader(form.Encode()))
			ExpectJSON(out, map[string]interface{}{
				"formula":    "($time_in_seconds / 60 / 60) * $memory_in_mb * 2",
				"id":         11,
				"name":       "ComputePlanB",
