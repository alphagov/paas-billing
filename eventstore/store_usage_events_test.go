	...
	*-----------------------------------------------------------------------------------*/
	It("should return one UsageEvent for each STARTED/STOPPED pair of RawEvent", func() {
		...
		Expect(usageEvents[0]).To(Equal(eventio.UsageEvent{
			...
			PlanUniqueID:  eventstore.StagingPlanGUID,
			PlanName:      "staging",
		...
		Expect(usageEvents[1]).To(Equal(eventio.UsageEvent{
			...
			SpaceGUID:     "276f4886-ac40-492d-a8cd-b2646637ba76",
			SpaceName:     "276f4886-ac40-492d-a8cd-b2646637ba76",
			PlanUniqueID:  eventstore.ComputePlanGUID,
			PlanName:      "app",
			ServiceGUID:   eventstore.ComputeServiceGUID,
			ServiceName:   "app",
			NumberOfNodes: 10,
		...
		Expect(usageEvents[2]).To(Equal(eventio.UsageEvent{
			...
			PlanUniqueID:  "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa",
			PlanName:      "Free",
	...
	It("should handle service UPDATE events that change the plan", func() {
		...
		Expect(events[1]).To(Equal(eventio.UsageEvent{
			...
			SpaceGUID:    "276f4886-ac40-492d-a8cd-b2646637ba76",
			SpaceName:    "276f4886-ac40-492d-a8cd-b2646637ba76",
			PlanUniqueID: "bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb",
			PlanName:     "PLAN2",
			ServiceGUID:  "efadb775-58c4-4e17-8087-6d0f4febc489",
			ServiceName:  "postgres",
		}))
	...
	*-----------------------------------------------------------------------------------*/
	It("should use the historic service data (name, label, uuid, unique_id) if available", func() {
		...
		Expect(usageEvents[0]).To(Equal(eventio.UsageEvent{
			...
			SpaceGUID:     "bd405d91-0b7c-4b8c-96ef-8b4c1e26e75d",
			SpaceName:     "bd405d91-0b7c-4b8c-96ef-8b4c1e26e75d",
			PlanUniqueID:  "c6221308-b7bb-46d2-9d79-a357f5a3837b",
	...
	*-----------------------------------------------------------------------------------*/
	It("should populate service info (name, label, uuid, unique_id) if historic data is not available", func() {
		...
		Expect(usageEvents[0]).To(Equal(eventio.UsageEvent{
			...
			SpaceGUID:     "bd405d91-0b7c-4b8c-96ef-8b4c1e26e75d",
			SpaceName:     "bd405d91-0b7c-4b8c-96ef-8b4c1e26e75d",
			PlanUniqueID:  "d5091c33-2f9d-4b15-82dc-4ad69717fc03",
			PlanName:      "myplan",
			ServiceGUID:   "efadb775-58c4-4e17-8087-6d0f4febc489",
