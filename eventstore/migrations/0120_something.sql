...
CREATE FUNCTION app_event_resource_state(raw_message jsonb) returns resource_state AS $$ BEGIN
	SELECT (raw_message->>'state')::resource_state;
END; $$ LANGUAGE SQL IMMUTABLE PARALLEL SAFE;

CREATE FUNCTION service_event_resource_state(raw_message jsonb) returns resource_state AS $$ BEGIN
	SELECT (case
...
CREATE FUNCTION task_event_resource_state(raw_message jsonb) returns resource_state AS $$ BEGIN
	SELECT (case
...
CREATE FUNCTION staging_event_resource_state(raw_message jsonb) returns resource_state AS $$ BEGIN
	SELECT (case
		when (raw_message->>'state') = 'CREATED' then 'STARTED'
		when (raw_message->>'state') = 'DELETED' then 'STOPPED'
		when (raw_message->>'state') = 'UPDATED' then 'STARTED'
...
CREATE FUNCTION app_event_filter(raw_message jsonb) returns BOOLEAN AS $$ BEGIN
...
CREATE FUNCTION service_event_filter(raw_message jsonb) returns BOOLEAN AS $$ BEGIN
	SELECT raw_message->>'service_instance_type' = 'managed_service_instance'
		and raw_message->>'space_name' !~ '^(SMOKE|ACC|CATS|PERF)-'; -- FIXME: this is open to abuse
END; $$ LANGUAGE SQL IMMUTABLE PARALLEL SAFE;

CREATE FUNCTION task_event_filter(raw_message jsonb) returns BOOLEAN AS $$ BEGIN
	SELECT (raw_message->>'state' = 'TASK_STARTED' or raw_message->>'state' = 'TASK_STOPPED')
		and raw_message->>'space_name' !~ '^(SMOKE|ACC|CATS|PERF)-'; -- FIXME: this is open to abuse
END; $$ LANGUAGE SQL IMMUTABLE PARALLEL SAFE;

CREATE FUNCTION staging_event_filter(raw_message jsonb) returns BOOLEAN AS $$ BEGIN
...
CREATE FUNCTION compose_service_event_filter(raw_message jsonb) returns BOOLEAN AS $$ BEGIN
	SELECT raw_message->>'space_name' !~ '^(SMOKE|ACC|CATS|PERF)-';
END; $$ LANGUAGE SQL IMMUTABLE PARALLEL SAFE;

CREATE FUNCTION service_instance_guid_if_created(raw_message jsonb) returns uuid AS $$ BEGIN
	SELECT (case
...
CREATE FUNCTION uuid_from_data_deployment(raw_message jsonb) returns uuid AS $$ BEGIN
	SELECT substring(
