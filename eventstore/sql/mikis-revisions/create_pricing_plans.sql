...
CREATE TABLE pricing_plan_components (
  pricing_plan_id INTEGER, -- simplify how components are joined to plans: use an autoincrement ID not a duplicate valid_from
  name text NOT NULL,
  ...
  PRIMARY KEY (pricing_plan_id, name),
  FOREIGN KEY (pricing_plan_id) REFERENCES pricing_plans (pricing_plan_id) ON DELETE CASCADE,
...
CREATE VIEW pricing_plan_components_with_vat_currency_with_duration AS (
  ...
  LEFT JOIN pricing_plan_components ppc ON
    ppc.pricing_plan_id = pp.pricing_plan_id
